...
- 目标：在当前系统支持范围内，优先使用最新 ABAP 语法，避免默认使用已淘汰旧写法。

1. `/init` 后先确认系统版本与语法能力边界。
2. 编码默认采用现代语法（内联声明、构造表达式、表表达式、Open SQL 新语法等）。
3. 如遇语法不兼容，先确认是版本限制还是代码问题。
4. 仅对不支持点做最小降级改写，不做整体回退。
5. 交付时附“降级点 + 原因 + 替代写法 + 后续升级建议”。
